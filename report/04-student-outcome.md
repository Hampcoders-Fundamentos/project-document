...
El presente documento corresponde al Trabajo Final (TB4) y tiene como alcance la consolidación completa de la arquitectura distribuida de Glottia, abarcando la migración total de 10 microservicios independientes: \textit{Identity and Access Management (IAM)}, \textit{Profiles}, \textit{Encounters}, \textit{Venues}, \textit{Promotions}, \textit{Learning Feedback}, \textit{Engagement}, \textit{Notifications}, \textit{Verification} y \textit{Analytics}. Estos servicios son orquestados mediante un \textit{API Gateway} (Spring Cloud Gateway) que actúa como perímetro de acceso, y soportados por un bus de mensajería asíncrona (RabbitMQ) que garantiza la comunicación desacoplada entre dominios. La arquitectura cloud se despliega en AWS (EC2, ECR, RDS, VPC) utilizando Terraform como Infrastructure as Code.

Se detalla la estrategia de aseguramiento de calidad implementada bajo el enfoque \textit{Behavior-Driven Development (BDD)} con una cobertura superior al 65\%, incluyendo escenarios Gherkin ejecutables para cada microservicio y reportes de ejecución generados mediante Cucumber. Las pruebas de integración validan los contratos OpenAPI de cada servicio, verificando códigos de estado HTTP y esquemas de respuesta JSON.

Finalmente, se presentan evidencias auditables del proceso de refactorización desde el monolito modular hacia microservicios, incluyendo la trazabilidad de commits, pipelines de integración continua, despliegue en infraestructura cloud y reportes de ejecución de los tres Sprints del proyecto.
...
- AV3: Profundicé en tecnologías de despliegue cloud y comunicación distribuida mediante la configuración de entornos para microservicios, el uso de RabbitMQ para integración basada en eventos y la preparación de servicios para su despliegue en AWS. \newline
...
- TP1: Profundicé en patrones de integración y despliegue continuo mediante herramientas IaaS como AWS EC2 y entornos Docker para asegurar la disponibilidad controlada de los microservicios core.\newline
